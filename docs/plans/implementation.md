...
└──────────────────────────────────────┘
```
Navigation: left/right edge tap = prev/next screen (Today ↔ Todos ↔ Meals ↔ Shopping ↔ Routine). Screen order fixed; current screen name in footer. AppLoad hosts the dashboard app in its own window; switching to xochitl for the Inbox notebook is a normal AppLoad app-switch (button/gesture per AppLoad's own UI — confirm exact mechanism when installed), not a launcher-specific hack.
...
**⏸ CHECKPOINT 3.1 — COMPLETE, recorded here for Claude Code's context (do not redo):** device shipped on OS **3.27.3.0**, far outside Toltec's supported ceiling (3.3.2.1666). Rather than downgrade firmware, this project uses **Vellum** (`vellum-dev/vellum-cli`), the actively-maintained successor that tracks current OS versions — no downgrade, no Toltec. Steps already done: SSH password recorded (Settings → Help → Copyright and licenses), automatic updates OFF, SSH bootstrap of vellum-cli over USB, `vellum check-os` confirmed all packages compatible. **Launcher: AppLoad (`vellum add appload`, currently `appload-0.5.3-r0`) — installed.** remux is Toltec-only and absent from Vellum's index, so it is not used anywhere in this project; every reference to a launcher in this document means AppLoad. Record the exact OS version and vellum package list in PROGRESS.md if not already there.
